...
class LabApp:
    def __init__(self, file_list):
        ...
        self.sl_prom = Slider(sl3, 'Порог Пика ', 0.1, 10.0, valinit=self.prominence)
    ...
    def save_report(self, event):
        ...
        with open(fname, "w", encoding="utf-8") as f:
            ...
            f.write(f"Настройки: MinDist={self.min_dist:.1f}, MaxDist={self.max_dist:.1f}, Prominence={self.prominence:.1f}\n\n")
            ...
            for d_item in self.data:
                ...
                f.write(f"{'№':<4} {'r_peaks (px)':<35} {'r^2':<35} {'delta':<10} {'Delta':<10} {'d/D':<8} {'Note'}\n")
                f.write("-" * 120 + "\n")
                ...
                for row in res['calc_data']:
                    ...
                    f.write(f"{row['k']:<4} {r_str:<35} {r2_str:<35} {delta_str:<10} {Delta_str:<10} {ratio_str:<8} {valid_mark} {note}\n")
                
                f.write("-" * 120 + "\n")
                if res['ratio']:
                    f.write(f"Среднее delta/Delta = {res['ratio']:.5f} +/- {res['ratio_err']:.5f}\n")
                    if d_item['B'] > 0.005:
                        Bs.append(d_item['B'])
                        Rs.append(res['ratio'])
            ...
            f.write("=== ИТОГОВЫЙ РАСЧЕТ (Линейная регрессия) ===\n")
            if len(Bs) > 1:
                Bs, Rs, Errs = np.array(Bs), np.array(Rs), np.array(Errs)
                try: 
                ...
                f.write(f"Магнетон Бора (эксперимент): {mu_calc:.4e} +/- {mu_err:.4e} Дж/Тл\n")
                f.write(f"Теоретическое значение:      {mu_B_theor:.4e} Дж/Тл\n")
                f.write(f"Отклонение от теории:        {diff_pct:.2f}%\n")
            else:
                f.write("Недостаточно точек для построения графика.\n")
