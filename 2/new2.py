...
# --- УЛУЧШЕННАЯ ФУНКЦИЯ ГРУППИРОВКИ (MIN и MAX) ---
def cluster_peaks(peaks, min_dist=5, max_dist=60):
    """
    Группирует пики. 
    min_dist: Если пики ближе этого значения -> это шум, второй игнорируется.
    max_dist: Если пики дальше этого значения -> это уже следующая группа (ордер).
    """
    if len(peaks) == 0: return []
    
    groups = []
    # Начинаем первую группу с первого пика
    ...
    for i in range(1, len(peaks)):
        ...
        # 1. Фильтр шума (слишком близко)
        if dist_from_last < min_dist:
            continue # Пропускаем этот пик, считаем его дребезгом
            
        # 2. Проверка принадлежности к текущей группе
        # Сравниваем с последним добавленным в группу компонентом
        if curr - current_group[-1] < max_dist:
            current_group.append(curr)
            last_valid_peak = curr
        
        # 3. Начало новой группы
        else:
...
class LabApp:
    def __init__(self, file_list):
        ...
        # Параметры группировки по умолчанию
        self.min_dist = 5   # Мин. расстояние (фильтр шума)
        self.max_dist = 65  # Макс. расстояние (размер группы)
        ...
        # 2. Слайдеры (теперь их 4)
        ax_sliders = self.fig.add_subplot(gs_ctrl[1])
        ax_sliders.axis('off')
        
        # Max Dist
        sl1 = ax_sliders.inset_axes([0, 0.75, 1, 0.15])
        self.sl_max = Slider(sl1, 'Max Dist (Группа) ', 5.0, 100.0, valinit=self.max_dist)
        self.sl_max.on_changed(self.update_params)

        # Min Dist: уменьшил диапазон до 0.1 - 20
        sl2 = ax_sliders.inset_axes([0, 0.55, 1, 0.15])
        self.sl_min = Slider(sl2, 'Min Dist (Шум) ', 0.1, 20.0, valinit=self.min_dist)
        self.sl_min.on_changed(self.update_params)
        
        # Prominence
        sl3 = ax_sliders.inset_axes([0, 0.35, 1, 0.15])
        self.sl_prom = Slider(sl3, 'Порог Пика ', 0.5, 20.0, valinit=3.0)
        self.sl_prom.on_changed(self.update_params)

        # Smooth
        sl4 = ax_sliders.inset_axes([0, 0.15, 1, 0.15])
        self.sl_smooth = Slider(sl4, 'Сглаживание ', 1, 21, valinit=5, valstep=2)
    ...
    def process_frame(self, i):
        ...
        # 1. Поиск пиков
        # distance=min_dist/2, чтобы find_peaks не отсеял раньше времени,
        # а наша функция cluster_peaks сама разобралась с min_dist
        ...
        # 2. Умная Группировка (Min - фильтр шума, Max - разбиение на порядки)
            ...
            # --- ДУБЛЕТ ---
                ...
                if len(group) == 2:
                    ...
                    valid = True
                     ...
                     # Если 3 пика в дублете, берем крайние (игнорируем шум посередине)
                    delta = (group[-1]**2 - group[0]**2) / 2.0
                    valid = False
                    note = "Лишний пик"
                else:
    ...
    def update_params(self, val):
        # Единый обработчик для всех слайдеров
    ...
    def update_view(self):
        ...
        lines = [f"ДАННЫЕ КАДРА ({self.mode})"]
        lines.append(f"Min: {self.min_dist:.1f} | Max: {self.max_dist:.1f}")
        lines.append("-" * 45)
    ...
    def save_report(self, event):
        ...
        with open(fname, "w", encoding="utf-8") as f:
            f.write("=== ОТЧЕТ ===\n")
            f.write(f"Settings: MinDist={self.min_dist}, MaxDist={self.max_dist}\n\n")
            
            Bs, Rs, Errs = [], [], []
            for d_item in self.data:
                f.write(f"--- ФАЙЛ: {d_item['name']} (I={d_item['I']} A) ---\n")
                res = d_item['res']
                if not res or not res['calc_data']: f.write("No Data\n"); continue
                
                for row in res['calc_data']:
                    valid = "+" if row['valid'] else "-"
                    f.write(f"Order {row['k']}: delta={row['delta']:.2f}, Delta={row['Delta'] if row['Delta'] else '-'} [{valid}] {row['note']}\n")
                
                if res['ratio']:
                    f.write(f"AVG d/D: {res['ratio']:.5f}\n")
                    if d_item['B'] > 0.005:
            ...
            if len(Bs) > 1:
                ...
                m = popt[0]
                mu_calc = m * (h * c) / (2 * d * n_ref)
                f.write(f"Mu_B Result: {mu_calc:.4e}\n")
