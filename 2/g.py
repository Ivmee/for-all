...
class LabApp:
    def __init__(self, file_list):
        ...
        # 2. Слайдер Порога
        ax_sl1 = self.fig.add_subplot(gs_ctrl[1])
        ax_sl1.axis('off')
        sl1 = ax_sl1.inset_axes([0, 0.6, 1, 0.3])
        self.sl_prom = Slider(sl1, 'Порог пиков ', 0.5, 20.0, valinit=3.0)
        self.sl_prom.on_changed(self.update_prominence)
        
        # 3. Слайдер Сглаживания
        ax_sl2 = self.fig.add_subplot(gs_ctrl[1]) 
        ax_sl2.axis('off')
        sl2 = ax_sl1.inset_axes([0, 0.2, 1, 0.3])
        self.sl_smooth = Slider(sl2, 'Сглаживание ', 1, 21, valinit=5, valstep=2)
    ...
    def process_frame(self, i):
        ...
        for k in range(n_groups):
            ...
            if self.mode == 'triplet':
                ...
                if k < n_groups - 1:
                    Delta = r_sq[(k+1)*3 + 1] - g_r2[1]
            else:
                delta = (g_r2[1] - g_r2[0]) / 2.0
                if k < n_groups - 1:
                    Delta = np.mean(r_sq[(k+1)*2 : (k+1)*2 + 2]) - np.mean(g_r2)

            valid = False
            # Фильтр
            if Delta is not None and delta > 0 and delta < Delta * 0.85:
                valid = True
                deltas.append(delta); Deltas.append(Delta)
            
            calc_data.append({'k': k+1, 'delta': delta, 'Delta': Delta, 'valid': valid, 'r_peaks': peaks_sub[idxs], 'r2': g_r2})
    ...
    def update_view(self):
        ...
        if res['calc_data']:
            for row in res['calc_data']:
                if row['valid']:
                    ...
                    self.ax_prof.text(rs[1], y_h+5, f"δ={row['delta']:.0f}", color='blue', ha='center', fontsize=8)
        ...
        lines.append("-" * 45)
        lines.append(f"{'№':<3}| {'delta':<8}| {'Delta':<8}| {'delta/Delta'}")
        lines.append("-" * 45)
        ...
        if res['calc_data']:
            for row in res['calc_data']:
                ...
                ratio_local = f"{(row['delta']/row['Delta']):.3f}" if (row['Delta'] and row['valid']) else "-"
                mark = " " if row['valid'] else "❌"
                lines.append(f"{row['k']:<3}| {d_val:<8}| {D_val:<8}| {ratio_local:<6} {mark}")
            
            lines.append("-" * 45)
            if res['ratio']:
                err_pct = (res['ratio_err']/res['ratio'])*100
                lines.append(f"СРЕДНЕЕ ОТНОШЕНИЕ: {res['ratio']:.4f}")
                lines.append(f"ПОГРЕШНОСТЬ:     ±{res['ratio_err']:.4f} ({err_pct:.1f}%)")
            else:
